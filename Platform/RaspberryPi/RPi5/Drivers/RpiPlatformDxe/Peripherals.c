  ...
  )
{
  ...
  IsD0 = FdtNodeOffsetByCompatible (Fdt, -1, "brcm,bcm2712d0-pinctrl") >= 0;
  if ((!IsD0 && (FdtNodeOffsetByCompatible (Fdt, -1, "brcm,bcm2712-pinctrl") < 0)) ||
      (FdtNodeOffsetByCompatible (Fdt, -1, IsD0
        ? "brcm,bcm2712d0-aon-pinctrl" : "brcm,bcm2712-aon-pinctrl") < 0)) {
    DEBUG ((DEBUG_ERROR, "GPIO: missing or inconsistent BCM2712 pinctrl nodes\n"));
    return EFI_UNSUPPORTED;
  }

  GpioInitialize (IsD0);
  DEBUG ((DEBUG_INFO, "GPIO: using BCM2712 %a register layout\n", IsD0 ? "D0" : "C1"));
